...
class LoggerRateLimiter {
     ...
     * - key: The message itself
     * - value: timeStamp + 10 seconds
     */
    class Logger {
        // 1. Init variable
        private val recordMap = HashMap<String, Int>()

        fun shouldPrintMessage(timestamp: Int, message: String): Boolean {
            // 2. Check variable
            val allowedTime = recordMap.getOrDefault(message, 0)
            if (allowedTime <= timestamp) {
                recordMap[message] = timestamp + 10
                return true
            }

            // If it is not allowed, just return false
            return false
        }
    }
